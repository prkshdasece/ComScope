...
static float score_data(unsigned char *data, int len) {
    ...
    if (max_run > 6) score += 20.0f;
    if (memchr(data, '\n', len) || memchr(data, '\r', len)) score += 15.0f;
    
    if (memchr(data, '\0', len)) score -= 30.0f; 
    if (ratio < 0.5f) score -= 40.0f;
...
int detect_baudrate(const char *port) {
    ...
    for (int pass = 0; pass < 2; pass++) {
        for (int i = 0; i < NUM_BAUDS; i++) {
            if (setup_serial_for_scan(fd, auto_baud_flags[i]) != 0) continue;
